   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +224610303003")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +224610303003 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./HIDARI SASAKI")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: SASAKI-COMPAGNIE `));
            console.log(chalk.magenta(`${themeemoji} GITHUB: WOLF2-07 `))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @SIGMA `))
